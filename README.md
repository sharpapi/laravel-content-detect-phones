...
This package ships a [Laravel Boost](https://github.com/laravel/boost) skill, `sharpapi-content-detect-phones`. It teaches AI coding agents the async submit-then-`fetchResults()` flow, the queued-job recipe, the result shape and the testing approach. Boost 2 or newer is required. In your app:
...
Select `sharpapi/laravel-content-detect-phones` when Boost lists the packages it found. The skill loads on demand; no always-on guideline is added.
